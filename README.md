...
赛题来源：[CANNJudge - BatchMatmulMaxSum](https://cannjudge.cn/public/op_challenge_shanghe_prelim/batchmatmulmaxsum)

## 赛题信息

| 项目 | 内容 |
| --- | --- |
| 算子 | BatchMatmulMaxSum |
| 题型 | 算子核函数工程（beta） |
| 计算类型 | cube |
| CANN 版本 | 9.0.0 |
...
BatchMatmulMaxSum 算子需要完成以下核心计算：
...
| --- | --- | --- | --- | --- | --- | --- |
| INPUT（必选） | x1 | tensor | 逻辑 shape 固定为 $(B,M,K)$；`transposeX1=false` 时 storage shape 为 $(B,M,K)$，`true` 时为 $(B,K,M)$ | FLOAT16、BFLOAT16 | ND | Query token embedding |
| INPUT（必选） | x2 | tensor | 逻辑 shape 固定为 $(B,K,N)$；`transposeX2=false` 时 storage shape 为 $(B,K,N)$，`true` 时为 $(B,N,K)$ | FLOAT16、BFLOAT16 | ND | Document token embedding 的矩阵乘布局 |
...
- 每个测试点单独计分。设 $T$ 为最优性能，$t$ 为当前提交性能，则单个测试点得分为：

$$
\mathrm{score}=\frac{100}{1+\log_{1.5}\left(\frac{t}{T}\right)}
$$
